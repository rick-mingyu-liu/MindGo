...
- **Terms**: [backend/utils/terms.js](backend/utils/terms.js) is the single definition of the Waterloo term calendar (Winter Jan–Apr, Spring May–Aug, Fall Sep–Dec). `GET /summary/rolling` takes **either** `?term=2026-spring|current|previous` **or** `?months=N`, never both, and echoes `term`, `termLabel`, `startDate`, `endDate` so a client never computes a date. The dashboard still sends `months=4` (`IMPROVEMENTS.md` item 22). Anything needing term boundaries — the summary views, and the retention job when it lands — must go through it rather than computing its own, because a view and a deletion that disagree about where a term starts would fail silently. Bounds are half-open `[start, end)`, matching the `date >= $1 AND date < $2` queries. **Do not build dates with `new Date(y, m, d).toISOString()`** anywhere: it is off by a day east of UTC (`getRollingSummary` still does — `IMPROVEMENTS.md` item 21).
