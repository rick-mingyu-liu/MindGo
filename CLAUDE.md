...
npm run lint           # eslint . (eslint 9, flat config in eslint.config.mjs, lints .ts too via typescript-eslint)
...
- **Unit tests always run**, with no database and no network: `exchangeRateService.test.js` (conversion, caching, and the failure modes that would silently produce a plausible wrong number), `configValidate.test.js` (the startup check, exercised in a child process because it calls `process.exit`), `packageRoot.test.js` (finding `backend/` from source or `dist/`), `errorSummary.test.js` (the only shape a caught error takes in a log), `cleanupService.test.js` (`db.query` mocked; asserts both retention predicates and that errors propagate), `schedulerService.test.js` (the interval plumbing, via `mock.timers`, and the weekly report's next run on a UTC clock), `aiUnavailable.test.js` (an OpenAI account out of credit or rate-limited answers 503 `ai_unavailable` on every AI endpoint, through the real router with a fake client), `privacy.test.js` (`maskEmail`, including every input that would make it throw inside a log line), `registerLogging.test.js` (runs the real `register` handler with stubbed collaborators and asserts no token or raw address reaches the log), `emailValidation.test.js` (every way MailboxLayer can fail to answer, each of which must degrade rather than block), `logger.test.js` (which levels survive `NODE_ENV=production`), `terms.test.js` (the term calendar, swept across four timezones in-process), `autoDeleteValidation.test.js` (mounts the real transactions router on an ephemeral port and asserts a rejected `months` never reaches `db.query`), `rollingSummary.test.js` (which window each request selects — term, year and rolling — through the real router, swept across four timezones), `dates.test.js` (the `DATE` type parser, the day helpers, and `monthlyBreakdown` keys asserted through the real router in three timezones), `importTokens.test.js`, `importRows.test.js`, `importClassify.test.js`, `importBankList.test.js`, and `importReceipt.test.js` (the screenshot parser's pieces, unit by unit), `importUberActivity.test.js` (Uber's trip list: date-and-time lines, buttons and map text ignored, category fixed to Transportation), `importUberEats.test.js` (Uber Eats' Past orders tab: squeezed order lines, store logos kept out of descriptions, category from the store alone), `importWechat.test.js` (WeChat Pay's list: yuan amounts with stray characters, years from month headers, direction from the sign), `importParse.test.js` (the parser end to end, against hand-built layouts in `test/helpers/ocrLayouts.js`), `importFixtures.test.js` (replays recorded OCR output from `eval/` through the real parser; fails on any silent amount or type error; it requires eval/lib/score.cjs, so a checkout without eval/ cannot run the backend suite), and `importRoutes.test.js` (both import endpoints through the real routers with `db.query` stubbed; asserts nothing logged contains screenshot text or row values, even when the database throws). `test/helpers/ocrLayouts.js` is not a test file and is not run — `npm test`'s `dist/test/**/*.test.js` glob does not match it.
...
- **Nothing guards the locale files against category drift.** Adding a
  transaction category means four edits (see above); `test/demoData.test.js`
...
- **The frontend has no test runner**, so `lib/date.ts` is unguarded. Its
  backend twin, `utils/dates.js`, is covered by `test/dates.test.js`.
