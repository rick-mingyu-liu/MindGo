/**
 * Copied verbatim from frontend/lib/date.ts on 2026-09-22.
 *
 * It ports across untouched because it never touches the DOM and never builds
 * a Date in order to name one. That matters more on mobile than on the web:
 * a phone's timezone follows the person carrying it, so the off-by-one-day
 * bug these helpers exist to prevent is not hypothetical here.
 *
 * If the frontend copy changes, change this one. Neither is tested — the
 * frontend has no test runner — but the backend twin, utils/dates.ts, is
 * covered by backend/test/dates.test.ts and defines the same behaviour.
 */
